...
export const saveConversation = (conversation) => {
  ...
  try {
    ...
    if (e.name === 'QuotaExceededError') {
      throw new Error("Storage full! Please delete some conversations.");
    }
    throw e;
  }
};
...
export const editMessage = (conversationId, messageId, newText) => {
    const conv = getConversation(conversationId);
    if (!conv) return;
    ...
    if (msgIndex !== -1) {
        ...
        conv.messages[msgIndex].edited = true; // Optional: mark as edited
        saveConversation(conv);
    }
    return conv;
};
